...
<head>
	...
	// Dark version	
	if($theme_options->get( 'dark_status' ) == 1) $lista_plikow[] = 'catalog/view/theme/'.$config->get('theme_' . $config->get('config_theme') . '_directory').'/css/dark.css';
    ...
    $lista_plikow[] = 'catalog/view/theme/'.$config->get('theme_' . $config->get('config_theme') . '_directory').'/js/jquery.matchHeight.min.js';
    ...
    if($theme_options->get( 'display_specials_countdown' ) == '1') {
         ...
         $lista_plikow[] = 'catalog/view/theme/'.$config->get('theme_' . $config->get('config_theme') . '_directory').'/js/jquery.countdown.min.js';
    }
...
<?php 		// For page specific css
		if (isset($request->get['route'])) {
		  ...
		  if($theme_options->get( 'always_show_vertical_menu_on_home' ) == '1') {
		       $klasa .= ' always_show_vertical_menu_on_home';
		  }
?>
<body class="<?php echo $klasa; ?>">
